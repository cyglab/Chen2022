...
% to calculate estimated time (te) based on BLS-U and BLS-C respectively. 
...
te1(:,:,isub)=boot.teConv;
E1(:,:,isub)=boot.EConv;
end;

bootBayes.teUnif=squeeze(te0);
bootBayes.para=squeeze(para0);
bootBayes.EUnif=squeeze(E0);

bootBayes.teConv=squeeze(te1);
bootBayes.EConv=squeeze(E1);

savepath='./';
save(strcat(savepath,'BayesUnifConv.mat'),'bootBayes');
clear bootBayes;
...
boot.teConv(1:3,iboot)=te3';
boot.teConv(4:6,iboot)=te4';
boot.EConv(:,iboot)=E1;
...
x= -10:0.001:10;
fun1 = @(x) x.*((normpdf(x,ts(1),ts(1).*ws)+normpdf(x,ts(2),ts(2).*ws)+normpdf(x,ts(3),ts(3).*ws))./3).*(normpdf(x,ts0,ts0.*ws));
y1=fun1(x);
q1=trapz(x,y1);


fun2 = @(x) ((normpdf(x,ts(1),ts(1).*ws)+normpdf(x,ts(2),ts(2).*ws)+normpdf(x,ts(3),ts(3).*ws))./3).*(normpdf(x,ts0,ts0.*ws));
y2=fun2(x);
q2=trapz(x,y2);
